...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    if (cartItems.length > 0) {
      const summary = calculateOrderSummary(cartItems, couponDiscount);
  ...
  const handleSubmit = async () => {
    if (!validateForm() || !orderSummary || !user) return;
    
    setIsSubmitting(true);
    
    try {
      const couponData = couponDiscount > 0 ? (await validateCoupon(couponCode, orderSummary.subtotal)).coupon : undefined;
                      ...
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => 
                            setCheckoutData(prev => ({ ...prev, saveAddress: checked as boolean }))
                          }
                        />
                    ...
                    <div key={item.id} className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-medium text-sm">
                          {formatVND(item.version === 'color'
                            ? (item.book.hasColorSale
                                ? (item.book.colorPrice - item.book.colorSaleAmount) * 1000 * item.quantity
                                : item.book.colorPrice * 1000 * item.quantity)
                            : item.book.photoPrice * 1000 * item.quantity
                          )}
                        </p>
                      </div>
                    </div>
